get '/search/anime/genres' do
  ...
  url = "/browse/anime?genres=#{ params['genre'].join(',') }&page=#{ params['page'] || 1 }"
  ...
  if !@search_results.empty?
    pagination_test_url = "/browse/anime?genres=#{ params['genre'].join(',') }&page=#{ (params['page']) ? params['page'].to_i + 1 : 1 + 1 }"
